...
- `scripts/audit_project.ps1` 已增加连接分组、RDB、访问日志、访问日志导出/筛选、连接历史、连接导入导出、搜索高亮、批量操作、全屏避让、工具箱入口、工具箱首批纯工具能力、工具箱网络工具能力、工作台内联主机列表、主题/语言偏好、主题色板、中英翻译主页面覆盖和顶部/底部玻璃层审计项；`scripts/audit_connection_groups.ps1` 已增加专项审计，并接入一键检查，覆盖分组页面、路由、首页入口/分组筛选、RDB-backed 仓库接口、改名、换色、排序、折叠和文档同步。2026-06-30 最新本地检查已通过全局审计 108/108、连接分组专项审计、终端解析器、Mock HAP 和 Real HAP 验包。

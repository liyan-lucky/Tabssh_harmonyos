...
| 工具箱/开发工具 | 网络拓扑、测速、IP、端口扫描、JSON、编码、二维码、取色、文本、单位转换 | `ToolboxPage` 已注册并能从工作台右上角和设置 Tab 进入；JSON 格式化/压缩、Base64 编解码、FNV-1a 快速校验、文本统计、颜色转换、单位换算、系统/存储/IP 基础信息、公网 IP、IP 详情全部网络/多网卡枚举、受控子网发现、访问审计跳转、默认网络/DNS/网关/路由/地址族摘要、TCP 连通性探测、ICMP 等价验收、端口扫描、HTTP 下载/上传测速、Nginx 配置摘要/同输入变量展开/include 检出/外部 include 导入展开和 QR Version 2-L 矩阵、二维码预览、SVG 保存写入/回读已编码；JSON、Encoding、网络拓扑、端口扫描、公网 IP、受控子网发现、HTTP 下载/上传测速、连通性、ICMP 等价验收、Nginx 同输入变量/include 摘要、外部 include 导入展开和 QR 矩阵、二维码预览、SVG 保存写入/回读和 IP 详情全部网络/多网卡枚举已取得 Real HAP 页面层级输出证据；PNG/相册保存、扫码兼容性和进一步美化仍未完成 |
...
本轮已取得 Mock/Real HAP 编译、Real HAP 安装、x86_64 hdc SSH HostKey/密码/PTY/命令输出、访问日志点击、导出选择器唤起、访问日志筛选空状态、连接历史点击、导入导出页点击、导入/导出 picker 唤起、工具箱入口、JSON/Encoding 工具面板输出、工具箱网络拓扑、端口扫描、HTTP 下载/上传测速、单项 TCP 连通性、ICMP 等价验收、Nginx 摘要/同输入变量展开/include 检出/外部 include 导入展开和 QR Version 2-L 矩阵输出、二维码预览、SVG 保存写入/回读、设置 Tab 展开/滚动、设置 Tab 系统语言跟随点击与强停重启回显、关于页版本构建时间、主题/语言源码覆盖、冷启动、基础新增分组与分组变更摘要跨重启回显、多页全屏 UI hierarchy 抽样、保存主机独立页入口、连接页最近历史和首页顶部渐变最终截图；但仍未取得完整设备点击、独立保存主机批量操作逐项验收、导入样本落库回显、导出文件内容回读、PNG/相册保存、扫码兼容性和进一步美化、完整主题/语言切换矩阵、arm64 真机和 schema migration 验收。字段/UI/RDB 未完成这些验收前，不能把对应 Android 功能标记为完成。

...
- 连接编辑页当前仍是深色表单风格，新增所属分组芯片选择，不引入新图标体系。
...
| ConnectionEditPage | 基础字段编辑、密码/私钥切换、私钥导入清理、所属分组芯片选择 | 校验增强、安全存储、代理/跳板机、分组选择设备验证 |
| ConnectionGroupPage | 分组列表、新建、改名、换色、上移/下移、折叠/展开、空分组处理、每组主机数；页面已注册路由但首页入口待接入 | RDB 持久化、拖拽排序、非空分组迁移、首页入口和设备验证 |

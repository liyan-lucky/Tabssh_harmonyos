...
- 筛选、排序、收藏、分组当前只绑定内存仓库；未接 RDB 前不能写成持久化功能。

| 页面 | 当前状态 | 真实功能要求 |
|---|---|---|
| Index | Mock/真实连接入口、连接搜索、收藏筛选、排序芯片、内存连接统计展示与导航 | RDB、分组入口、批量编辑、在线状态、搜索高亮、统计页 |
| ConnectionEditPage | 基础字段编辑 | 校验、安全存储、代理/跳板机、分组选择 |
| ConnectionGroupPage | 分组列表、新建分组、折叠/展开、删除空分组、每组主机数；页面已注册路由但首页入口待接入 | RDB 持久化、重命名、排序拖拽、非空分组迁移、首页入口和设备验证 |

...
4. 补齐工具箱剩余真实工具能力，优先使用纯 HarmonyOS API 继续完善二维码 SVG 真实目标写入/回读、PNG/相册保存、扫码兼容性和进一步美化，以及外部 Nginx include 文件导入/展开；ICMP 目前以普通应用可用的默认网络、DNS 解析和 TCP connect 做等价验收，只有 HarmonyOS API 无法实现时才考虑三方库。

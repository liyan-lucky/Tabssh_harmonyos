...
- 点击网络测速，使用 HTTP/HTTPS URL 做下载样本测速；当前不测上传速度，输出中必须说明上传待补。
- 点击 Nginx 拓扑，确认能汇总 upstream、server、listen、server_name 和 proxy_pass；复杂 include/变量展开仍待补。
- 点击二维码工具，确认输出待编码负载摘要；当前还不是二维码图片矩阵或美化结果。
...
当前工具箱已有首批纯 ArkTS 开发/系统小工具和纯 HarmonyOS 网络工具子集；网络拓扑和端口扫描已有设备输出。HTTP 测速、连通性、Nginx 和 QR 负载摘要仍需逐项设备点击，主动子网发现、上传测速、特权 ICMP、二维码图片矩阵、公网 IP、更多网卡字段和复杂 Nginx include/变量展开仍未完成。主题和多语言当前已覆盖主壳、工作台、设置 Tab、设置页、工具箱页、主要二级页、连接编辑、终端、SFTP 和端口转发；仍需跨重启、系统语言跟随和无障碍/高对比验证。
...
- 工具箱中的 HTTP 测速、单项 TCP 连通性、Nginx 摘要和 QR 负载摘要逐项设备点击证据，以及主动子网发现、上传测速、特权 ICMP、二维码图片矩阵、公网 IP、更多网卡字段和复杂 Nginx include/变量展开。
- 全局主题/多语言完整矩阵；当前已完成主壳、工作台、设置 Tab、设置页、工具箱页、关于、终端设置、连接历史、访问日志、连接分组、连接导入导出、连接编辑、终端、SFTP 和端口转发页，仍需跨重启、系统语言跟随、无障碍/高对比和动态文案验收。

...
- 首页工作台主机区域只应显示新增主机、已保存主机和连接历史等入口，不应显示已保存主机的名称、用户、主机、端口、状态或连接按钮。
- 点击“已保存主机”应进入 `SavedHostsPage` 独立页面；新增主机仍应进入原 `ConnectionEditPage`。
...
- 点击二维码工具，确认输出 QR Version 2-L Byte、码字统计和 `##/..` 矩阵；图片保存和视觉美化仍待补。

...
8. 安装 HAP 后优先验证：首页工作台只显示主机管理入口且不泄露主机详情、`SavedHostsPage` 独立保存主机管理、连接 Tab 最近 10 条历史、右上工具箱入口、设置 Tab 工具箱入口、工具箱网络拓扑/端口扫描/测速/连通性/Nginx/QR 工具、设置 Tab 主题/语言切换、连接分组页路由编译、导入导出页 picker 与真实文件回读、顶部/底部半透明玻璃层、Terminal/SFTP/PortForward/Settings/About 路由。

...
- 工具箱剩余网络类工具能力：网络拓扑、默认网络信息、端口扫描、公网 IP 和受控子网发现已有 Real HAP 输出；仍需补 HTTP 下载测速、单项 TCP 连通性、Nginx 摘要和 QR 负载摘要的逐项点击证据，以及上传测速、特权 ICMP、二维码图片矩阵/美化、更多网卡字段和复杂 Nginx include/变量展开。
